...
export default function Gallery({ title, images, imagePosition = 'center', imageFit = 'cover' }) {
  ...
  return <section className="gallery-section">
    ...
    <div className="gallery">{visibleImages.map((image, i) => {
      ...
      return <button type="button" key={image} onClick={(event) => { selectedButton.current = event.currentTarget; open(imageIndex) }}><img src={image} alt={`웨딩 사진 ${imageIndex + 1}`} loading="lazy" onError={() => removeBroken(image)} style={{ objectPosition: imagePosition, objectFit: imageFit }} /></button>
    })}</div>
